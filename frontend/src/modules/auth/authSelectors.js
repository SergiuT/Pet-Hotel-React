...
const selectCurrentUserIsPetOwner = createSelector([selectCurrentUser], (currentUser) => {
  return !new PermissionChecker(currentUser).rolesMatchOneOf([
...
const selectCurrentUserIsManager = createSelector(
  [selectCurrentUser],
  (currentUser) => {
    return new PermissionChecker(currentUser).rolesMatchOneOf(Roles.values.manager);
  });
...
const selectRedirectToNewPet = createSelector([selectRaw], (raw) => {
