...
class BookingViewItem extends Component {
  ...
  label = (record) => {
    const arrival = bookingModel.fields.arrival.forView(record.arrival);
    const departure = bookingModel.fields.departure.forView(record.departure);
    const status = bookingModel.fields.status.forView(record.status);
