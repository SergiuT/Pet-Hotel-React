...
class BookingForm extends Component {
  ...
  statusOptions = () => {
    const { isPetOwner, isManager, isEmployee } = this.props;
  ...
  statusOptionsEmployee = () => {
    ...
    return options.filter((option) => option.id !== bookingStatus.BOOKED);
  }
  ...
  onPeriodChange(value, form) {
    ...
    this.calculateAndSetFree(value[0], value[1], form);
  }

  calculateAndSetFree = (arrival, departure, form) => {
    const { dailyFee } = this.props;

    const fee = BookingFeeCalculator.calculator(
      arrival,
      departure,
      dailyFee
    );
          ...
          render={(form) => {
            return (
              <Form onSubmit={form.handleSubmit}>
                ...
                {this.isOwnerEnabled() && (<UserAutocompleteFormItem
                ...
                {this.isPetEnabled(form) && (<PetAutocompleteFormItem
                  name={fields.pet.name}
                  label={fields.pet.label}
                  required={fields.pet.required}
                  owner={form.values.owner ? form.values.owner.id : null}
                />)}
                ...
                />
                {this.isEmployeeNotesAndPhotosEnabled(form) && (
                  <React.Fragment>
                    <TextAreaFormItem
                ...
                {this.isStatusEnabled() && (<SelectFormItem
                ...
                {this.isFeeVisible(form) && (<ViewFormItem
                ...
                >
                  <Button
                    loading={saveLoading || form.isSubmitting}
                    type="primary"
                    htmlType="submit"
                    icon="save"
                  >
                  ...
                  <Button
                    disabled={saveLoading || form.isSubmitting}
...
function select(state) {
  return {
    findLoading: selectors.selectFindLoading(state) || settingsSelectors.selectFindLoading(state),
    saveLoading: selectors.selectSaveLoading(state),
    record: selectors.selectRecord(state),
    isPetOwner: authSelectors.selectCurrentUserIsPetOwner(state),
    isManager: authSelectors.selectCurrentUserIsManager(state),
    IsEmployee: authSelectors.selectCurrentUserIsEmployee(state),
