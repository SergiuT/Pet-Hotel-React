...
class BookingRepository extends AbstractEntityRepository {
  ...
  async findAllAutocomplete(filter, limit) {
    ...
    if (filter && filter.owner) {
      rows = rows.filter((row) => row.owner === filter.owner);
    }
  ...
  async countActiveBookingsInPeriod(start, end, idToExclude) {
    ...
    const arrivalFilter = (item) => moment(item.arrival).isSameOrBefore(end);

    const statusFilter = (item) => [bookingStatus.BOOKED, bookingStatus.PROGRESS].includes(item.status);

    const idToExcludeFilter = (item) => !idToExclude || item.id !== idToExclude;

    return results.filter(arrivalFilter).filter(idToExcludeFilter).filter(statusFilter).length;
  }
}
